...
export default function ProductAttributes({
  ...
  const displayEntries = showAll
    ? attributeEntries
    : attributeEntries.slice(0, maxInitialDisplay);
  ...
  const renderAttributeValue = (
    ...
    if (typeof value === "object" && value !== null) {
      ...
      return (
        <div
          className={`space-y-3 ${
            level > 0 ? "pl-4 border-l-2 border-muted/50" : ""
          }`}
        >
          {hasNestedContent && (
            <Button
  ...
  const getAttributeIcon = (key: string) => {
    ...
    if (
      ...
      keyLower.includes("taille") ||
      keyLower.includes("hauteur") ||
      keyLower.includes("largeur") ||
      keyLower.includes("profondeur") ||
      keyLower.includes("cm") ||
      keyLower.includes("mm")
    ) {
      return <Ruler className="h-4 w-4 text-blue-500" />;
    }
    if (
      keyLower.includes("finition") ||
      keyLower.includes("couleur") ||
      keyLower.includes("color")
    ) {
      return <Palette className="h-4 w-4 text-purple-500" />;
    }
    if (
      keyLower.includes("niveau") ||
      keyLower.includes("layer") ||
      keyLower.includes("etage")
    ) {
    ...
    if (
      keyLower.includes("performance") ||
      keyLower.includes("puissance") ||
      keyLower.includes("vitesse")
    ) {
      return <Zap className="h-4 w-4 text-yellow-500" />;
    }
    if (
      keyLower.includes("securite") ||
      keyLower.includes("protection") ||
      keyLower.includes("certification")
    ) {
      return <Shield className="h-4 w-4 text-red-500" />;
    }
    if (
      keyLower.includes("qualite") ||
        ...
        >
          <CardContent className="p-4">
            <div className="space-y-3">
              ...
              <div className="ml-7">{renderAttributeValue(key, value)}</div>
            </div>
          </CardContent>
        </Card>
      ))}
