...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      brand.description || `Explore ${brand.name} products and categories.`,
  };
}
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                ...
                {brand._count?.catalogues && brand._count.catalogues > 0 && (
                  <Badge variant="outline" className="text-sm">
                    {brand._count.catalogues} Catalogue
                    {brand._count.catalogues !== 1 ? "s" : ""}
                  </Badge>
                )}
              </div>
            </div>
          ...
          {categories.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories.map((category) => (
                ...
                >
                  <Link
                    href={`/products?brand=${brand.id}&category=${category.id}`}
                  >
